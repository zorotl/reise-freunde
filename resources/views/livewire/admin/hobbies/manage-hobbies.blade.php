<div>
    ...
    <div class="mb-4 p-4 bg-gray-100 dark:bg-zinc-700 rounded-lg shadow-sm">
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
            {{ $editingHobbyId ? __('Edit Hobby') : __('Add New Hobby') }}
        </h3>
        <form wire:submit.prevent="saveHobby">
            <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-grow">
                    <label for="name" class="sr-only">{{ __('Hobby Name') }}</label>
                    <input type="text" wire:model="name" id="name" placeholder="{{ __('Hobby Name') }}"
                ...
                <div class="flex-shrink-0 flex gap-2">
                    <button type="submit"
                        ...
                        {{ $editingHobbyId ? __('Save Changes') : __('Add Hobby') }}
                    </button>
                    @if ($editingHobbyId)
    ...
    <div class="mb-4 flex justify-between items-center">
        <div class="flex-1 me-4">
            <input wire:model.live="search" type="text" placeholder="{{ __('Search hobbies...') }}"
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow overflow-hidden overflow-x-auto"> {{-- Added overflow-x-auto
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
            ...
            <tbody class="bg-white dark:bg-zinc-800 divide-y divide-gray-200 dark:divide-neutral-700">
                ...
                <tr @if($hobby->trashed()) class="bg-red-100 dark:bg-red-900/50 opacity-75" @endif>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium w-48"> {{-- Adjust width as
                        needed --}}
                        {{-- Action Buttons --}}
                        @if ($editingHobbyId !== $hobby->id) {{-- Hide Edit button if currently editing this item --}}
                        <button wire:click="editHobby({{ $hobby->id }})"
                        ...
                        <button wire:click="restoreHobby({{ $hobby->id }})"
                            onclick="return confirm('{{ __('Are you sure you want to restore this hobby?') }}')"
                            class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-600 me-3">{{
                            __('Restore') }}</button>
                        <button wire:click="forceDeleteHobby({{ $hobby->id }})"
                            onclick="return confirm('{{ __('Are you sure you want to permanently delete this hobby? This action cannot be undone.') }}')"
                            class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-600">{{
                            __('Force Delete') }}</button>
                        @else
                        {{-- Show soft delete option if not soft deleted --}}
                        <button wire:click="softDeleteHobby({{ $hobby->id }})"
                            onclick="return confirm('{{ __('Are you sure you want to soft delete this hobby?') }}')"
